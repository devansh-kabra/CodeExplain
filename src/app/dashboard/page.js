...
//   }
    ...
    return (
        <div id="dashboard" className="bg-[#1A1F25] min-h-dvh flex flex-col gap-4">
            ...
            <section id="stats" className="w-full flex justify-center mt-2">
                <div className="grid grid-cols-1 sm:grid-cols-3 rounded-sm w-[90%] sm:w-[85%] lg:w-[75%] gap-2 bg-[#1C2229] ">
                    ...
                    <Suspense fallback={<Loading/>}>
                        <TotalProgress userId={userId} total={400}/>
                    </Suspense> 
                    <div id="weekly-progress" className="dashboard_kpi_boxes flex flex-col items-center justify-center gap-2">
                        <h2 className="text-[#DDE3EA] font-poppins text-lg sm:text-base md:text-lg text-center">Streak: 21🔥</h2>
                        <Suspense fallback={<Loading/>}>
                            <WeeklyProgress/>
                        </Suspense>
                    </div>  
